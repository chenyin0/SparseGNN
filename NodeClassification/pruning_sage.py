...
def get_mask_distribution(model, if_numpy=True):
    ...
    weight_mask_tensor0 = model.net_layer[0].lin_l.weight_mask_train.flatten()  # 22928
...
def random_pruning(model, adj_percent, wei_percent):
    ...
    for i, j in wei1_pruned:
        model.net_layer[0].lin_l.weight_mask_train[i][j] = 0
...
def add_trainable_mask_noise(model, c):
    ...
    rand2 = (2 * torch.rand(model.net_layer[0].lin_l.weight_mask_train.shape) - 1) * c
    rand2 = rand2.to(model.net_layer[0].lin_l.weight_mask_train.device)
...
def soft_mask_init(model, init_type, seed):
    ...
    if init_type == 'all_one':
        ...
        model.adj_mask1_train.requires_grad = True
        init.kaiming_uniform_(model.net_layer[0].lin_l.weight_mask_train, a=math.sqrt(5))
        ...
        model.adj_mask1_train.mul_(model.adj_mask2_fixed)
        model.adj_mask1_train.requires_grad = True
        init.normal_(model.net_layer[0].lin_l.weight_mask_train, mean=mean, std=std)
        ...
        model.adj_mask1_train.mul_(model.adj_mask2_fixed)
        model.adj_mask1_train.requires_grad = True
        init.uniform_(model.net_layer[0].lin_l.weight_mask_train, a=a, b=b)
