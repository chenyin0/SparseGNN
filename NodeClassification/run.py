...
for dataset in dataset_list:
    for model in model_list:
        if model == 'gcn' or model == 'graphsage':
            os.system('/usr/bin/python3.8 main_pruning_imp.py' + ' --net=' + model + ' --dataset=' +
                      dataset)
        elif model == 'gin':
            os.system('/usr/bin/python3.8 main_gingat_imp.py' + ' --net=' + model + ' --dataset=' +
                      dataset)
